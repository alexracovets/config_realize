const garmentNameMapFragment = `
  for ( int nameSlot = 0; nameSlot < 4; nameSlot ++ ) {
    ...
    slotName = garmentCompositeNameLayer( slotName, uNameStrokeColors[ nameSlot ], garmentNameSampleStrokeChannel( uNameMask, nameStampUv, nameChannel, uNameStampSize ) * nameInside );
    slotName = garmentCompositeNameLayer( slotName, uNameTextColors[ nameSlot ], garmentNameSampleFillChannel( uNameMask, nameStampUv, nameChannel, uNameStampSize ) * nameInside );
